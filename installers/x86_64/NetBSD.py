...
NAME = "NetBSD x86_64"
DESCRIPTION = "Installer for NetBSD running on an emulated x86_64 Processor"


def setup(_):
    """Walk the user through setting up an NetBSD x86_64 environment
    """
    print()
    ...
    with urllib.request.urlopen("http://ftp.netbsd.org/pub/NetBSD/") as f:
        html = f.read()


    # Parse it out
    versions = [x.decode('ascii') for x in re.findall(b'NetBSD-([0-9\.]+)',html)]
    ...
    url = "http://ftp.netbsd.org/pub/NetBSD/NetBSD-{0}/amd64/installation/cdrom/boot.iso".format(version)
