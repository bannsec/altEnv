...
NAME = "OpenBSD x86_64"
DESCRIPTION = "Installer for OpenBSD running on an emulated x86_64 Processor"


def setup(_):
    """Walk the user through setting up an OpenBSD x86_64 environment
    """
    print()
    ...
    with urllib.request.urlopen("http://ftp3.usa.openbsd.org/pub/OpenBSD/") as f:
        html = f.read()
    ...
    url = "http://ftp3.usa.openbsd.org/pub/OpenBSD/{0}/amd64/cd{1}.iso".format(version,version.replace(".",""))
